...
void Selecao(Game game[], int tamanho, long *comparacoes, long *movimentacoes) {
    *comparacoes = 0;
    *movimentacoes = 0;
    ...
    for (int i = 0; i < tamanho - 1; i++) {
        ...
        for (int j = i + 1; j < tamanho; j++) {
            (*comparacoes)++;
            if (strcmp(game[menor].nome.str, game[j].nome.str) > 0) {
                menor = j;
            }
        }
        if (i != menor) {
...
int main() {
    ...
    Game *gameSelecao = (Game*)malloc(100 * sizeof(Game)); // sub array de games 
    
    if(gameSelecao==NULL){return 1;}
    int tamanho=0;
    String busca;
    scanf("%s", busca.str);
    ...
    free(game);
    gameSelecao = (Game*)realloc(gameSelecao, tamanho*sizeof(Game)); //realloc 
   
    long comparacoes = 0, movimentacoes = 0;
    clock_t start, end;

    start= clock();
    Selecao(gameSelecao, tamanho, &comparacoes, &movimentacoes);
    end = clock();

    FILE *log  =fopen("878672_selecao.txt", "w");
     if (log != NULL) {
    ...
    for(int i=0; i<tamanho; i++){imprimir(&gameSelecao[i]);}
    free(gameSelecao);
    return 0;
}