...
private void reconstruir(int tamHeap, int i) {
    int maior = i;
    int esquerda = 2 * i + 1;
    int direita = 2 * i + 2;

    if (esquerda < tamHeap) {
        if (games[esquerda].getEstimatedOwners() > games[maior].getEstimatedOwners() ||
    ...
    if (direita < tamHeap) {
        if (games[direita].getEstimatedOwners() > games[maior].getEstimatedOwners() ||
...
public class GameMainHeap {
    public static void main(String[] args)  throws FileNotFoundException {
        ...
        int tamanhoHeap=0;
        Leitura[] prontoHeap = new Leitura[300];
